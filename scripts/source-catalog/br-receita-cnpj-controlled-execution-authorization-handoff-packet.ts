/**
 * BR Receita CNPJ — controlled execution authorization handoff packet CLI (BR-SOURCE-13I).
 *
 * Prints the controlled execution authorization handoff packet for a named SYNTHETIC fixture and a
 ...
 *
 * This CLI has exactly three inputs — a fixture NAME, a decision VALUE and a FORMAT — and it has no code
 * path that accepts a location: there is no `--manifest`, no `--input`, no `--output`, no `--path`, no
 * `--real-data`, and no way to point it at a byte of the real dataset. It also has no `--execute`, no
 * `--run`, no `--apply`, no `--force`, no `--import`, no `--activate` and no `--approve`, because it has
 * nothing to run, nothing to switch on and nothing to approve: it prints a `NO_GO` packet and exits. It
 * writes to stdout only; it never creates a file.
 *
 * `--approve` deserves its own note. It is the flag this CLI would most plausibly be expected to have —
 * the packet it prints is a list of nine decisions, so a reader may reasonably look for the switch that
 * answers one. There is none, and there will not be one. An approval is a human act recorded outside this
 * repository; a command-line flag that claimed to grant one would be the single most dangerous argument
 * in the whole chain.
 *
 * ── Central rule ─────────────────────────────────────────────────────────────
 ...
 *   - hands anything over to anyone: it prints a document, and a printed document is not a delivered
 *     decision.
 *   - approves a gate, authorizes a cap, or marks Brazil ready for the full join, import, runtime or
 *     Agent 1.
 *
 * Its only inputs are the command-line arguments; its only outputs are stdout (the packet) and stderr (a
 * usage error). The packet carries a STATIC timestamp, so two identical runs are byte-identical.
 *
 * Usage:
...
import {
  ...
  BRAZIL_RECEITA_HANDOFF_FORMATS,
  ...
  type BrazilReceitaControlledExecutionAuthorizationHandoffFormat,
} from '../../src/server/source-catalog/connectors/br-receita-cnpj/br-receita-cnpj-controlled-execution-authorization-handoff-packet';
import {
...
/**
 * Flags that would turn this into a real-data, an executing, an activating or an APPROVING tool. Their
 * mere presence is a fail-closed usage error — this generator writes a synthetic `NO_GO` packet and
 * nothing else: it accepts no path, no payload, no manifest and no real-data mode, it has no run,
 * execute, apply, import or activate behaviour to reach, it cannot approve any of the nine decisions it
 * describes, and `--force` is listed for the same reason as the rest, since there is no refusal here that
 * a flag could override — the refusal is the output.
 */
...
export const HANDOFF_USAGE_ERROR_CODES = {
  forbiddenFlag: 'BRSOURCE13I_FORBIDDEN_ARGUMENT',
  unknownFlag: 'BRSOURCE13I_UNKNOWN_ARGUMENT',
  fixtureMissing: 'BRSOURCE13I_FIXTURE_REQUIRED',
  fixtureUnknown: 'BRSOURCE13I_FIXTURE_UNKNOWN',
  decisionMissing: 'BRSOURCE13I_DECISION_REQUIRED',
  decisionUnknown: 'BRSOURCE13I_DECISION_UNKNOWN',
  formatMissing: 'BRSOURCE13I_FORMAT_REQUIRED',
  formatUnknown: 'BRSOURCE13I_FORMAT_UNKNOWN',
} as const;

// ─── Types ───────────────────────────────────────────────────────────────────

export type HandoffCliOptions = {
  ...
  readonly format: BrazilReceitaControlledExecutionAuthorizationHandoffFormat;
  readonly pretty: boolean;
};

/** A usage error carrying its own code, so `main` never has to classify a message. */
export class HandoffUsageError extends Error {
...
function fixtureList(): string {
  return BRAZIL_RECEITA_HANDOFF_FIXTURE_NAMES.join(', ');
}
...
function formatList(): string {
  return BRAZIL_RECEITA_HANDOFF_FORMATS.join('|');
}
 ...
 * optional `--pretty`. Everything else is rejected: a forbidden flag first (with the reason it can never
 * be supported), then any unrecognized argument, positionals included.
 *
 * @throws HandoffUsageError on any input this CLI does not accept.
 */
export function parseHandoffArgs(argv: readonly string[]): HandoffCliOptions {
  ...
  let format: BrazilReceitaControlledExecutionAuthorizationHandoffFormat | undefined;
  let pretty = false;
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if ((FORBIDDEN_FLAGS as readonly string[]).includes(flag)) {
      throw new HandoffUsageError(
        HANDOFF_USAGE_ERROR_CODES.forbiddenFlag,
        `${flag} is not supported and never will be. This handoff packet generator writes a synthetic NO_GO packet only: it accepts no path, no payload, no manifest and no real-data mode, it cannot run, execute, apply, import, activate or force anything, it cannot approve or authorize any of the nine decisions it describes, and it writes to stdout only. Accepted flags: ${ALLOWED_FLAGS.join(', ')}.`,
      );
    }
    ...
    if (flag === '--fixture') {
      ...
      if (read.value === undefined) {
        throw new HandoffUsageError(
          HANDOFF_USAGE_ERROR_CODES.fixtureMissing,
      ...
      if (!isBrazilReceitaRequestPacketFixtureName(read.value)) {
        throw new HandoffUsageError(
          HANDOFF_USAGE_ERROR_CODES.fixtureUnknown,
    ...
    if (flag === '--decision') {
      ...
      if (read.value === undefined) {
        throw new HandoffUsageError(
          HANDOFF_USAGE_ERROR_CODES.decisionMissing,
      ...
      if (!isBrazilReceitaReviewDecisionValue(read.value)) {
        throw new HandoffUsageError(
          HANDOFF_USAGE_ERROR_CODES.decisionUnknown,
    ...
    if (flag === '--format') {
      ...
      if (read.value === undefined) {
        throw new HandoffUsageError(
          HANDOFF_USAGE_ERROR_CODES.formatMissing,
      ...
      if (!isBrazilReceitaRequestPacketFormat(read.value)) {
        throw new HandoffUsageError(
          HANDOFF_USAGE_ERROR_CODES.formatUnknown,
    ...
    throw new HandoffUsageError(
      HANDOFF_USAGE_ERROR_CODES.unknownFlag,
...
/**
 * Exit code 0 means a packet was produced — and every packet is `NO_GO`, because a refusal to declare an
 * authorization is the correct outcome for this tool and the refusal itself is the artefact worth
 * reading. A `NO_GO` packet is not an error, so it is never reported as one. Exit code 1 means the
 * ARGUMENTS were unusable and nothing was produced.
 */
export function main(argv: readonly string[] = process.argv.slice(2)): void {
  let options: HandoffCliOptions;
  try {
  ...
  const rendered = formatBrazilReceitaControlledExecutionAuthorizationHandoffPacket(
    packet,
    options.format,
    options.pretty,
  );
...
if (
  executedFile.endsWith('br-receita-cnpj-controlled-execution-authorization-handoff-packet.ts')
) {
  main();
}
