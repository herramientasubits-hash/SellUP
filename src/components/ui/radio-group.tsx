 ...
 *
 * Combines Base UI's RadioGroup root (`@base-ui/react/radio-group`) and
 * the Radio item primitives (`@base-ui/react/radio`):
 *   - RadioGroup      : root container, renders <div>, manages selection
 *   - RadioGroupItem  : individual radio, renders <span> + hidden <input>
 *
 * Keyboard: arrow keys move focus between items, Space/Enter selects.
 * Selection state is exposed via the `data-checked` attribute.
 */
function RadioGroup({
  className,
  ...props
