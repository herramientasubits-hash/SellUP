...
describe('H2 — files drop the legacy enrichment_configuration_unavailable error', () => {
  ...
  // slack-connection.ts, the two slack OAuth route handlers, the auth callback
  // route, and the integrations actions module all threw
  // `enrichment_configuration_unavailable` from their inline getAdminSupabase().
  // The fail-closed factory now throws UnsafeSupabaseEnvironmentError instead,
  // so the legacy string must not survive (as code or as a live throw) in any
  // migrated file.
