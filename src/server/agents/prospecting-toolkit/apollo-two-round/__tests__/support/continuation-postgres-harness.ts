...
export const MINIMAL_DEPENDENCIES_SQL = `
  ...
  -- La columna metadata es dependencia MINIMA de dos cosas: la FK de la 139 y
  -- el presupuesto durable de la corrida, que vive en ese JSONB con su CAS.
...
export function postgrestShim(run: QueryRunner) {
  return {
    ...
    from: (table: string) => ({
      // Lectura `.select(cols).eq(col, v).maybeSingle()`, la que usa el
      // presupuesto durable para releer antes de cada CAS.
      ...
      update: (patch: Record<string, unknown>) => {
        ...
        const builder = {
          ...
          async select(_columns?: string) {
            ...
            // Las rutas PostgREST `metadata->clave->>campo` se traducen a la
            // sintaxis de Postgres; es transporte, no regla.
            const toSql = (column: string): string =>
              column.includes('->')
                ? column
                    ...
                    .replace(/->([A-Za-z_]+)(?=->|$)/g, "->'$1'")
                : column;
            let param = keys.length;
            const whereParts: string[] = [];
            const filterValues: unknown[] = [];
            for (const f of filters) {
              if (f.isNull) {
                whereParts.push(`${toSql(f.column)} IS NULL`);
                continue;
              }
              param += 1;
              whereParts.push(`${toSql(f.column)} = $${param}`);
