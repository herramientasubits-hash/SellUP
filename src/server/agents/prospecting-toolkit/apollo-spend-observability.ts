 ...
 * were the ones we could say least about after the fact.
 *
 * This module builds that record once, from whatever is available, and both
 * paths persist the same shape. Every field is nullable on purpose: an absent
 * header must be recorded as "we did not receive it", never as 0, which would
 * be indistinguishable from "the quota is exhausted".
 *
...
/**
 * Flat, nullable observability record persisted in
 * provider_usage_logs.metadata.apollo_spend_observability.
 */
...
export const APOLLO_SPEND_OBSERVABILITY_KEY = 'apollo_spend_observability' as const;
