 ...
 *
 * `rawDiscovered` es lo que el descubrimiento entregó; `acceptedUnique` es lo
 * único que puede contar contra el objetivo global del lote. Confundirlos es
 * cómo un lote de 10 se llena con 10 filas de tres empresas.
 */
...
/**
 * Aplica una decisión a los contadores y devuelve unos NUEVOS.
 *
 * `rawDiscovered` sube SIEMPRE (el candidato llegó). `errors` NO se toca nunca
 * aquí: un duplicado no es un fallo de escritura.
 */
export function tallyBatchIdentityDecision(
  ...
  return {
    ...
    acceptedUnique:
      counters.acceptedUnique + (decision.action === 'hard_duplicate' ? 0 : 1),
    duplicateSkipped:
...
export function toBatchIdentityCountersMetadata(
  ...
  return {
    ...
    accepted_unique: counters.acceptedUnique,
