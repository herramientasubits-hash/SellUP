 ...
 *   QA batch of 2026-07-30 was charged 4 credits (3 organizations_search +
 *   1 organization_enrichment) against a 3-credit reservation. The reservation
 *   only knew about one of the two billable operations.
 *
 * Contract:
 *   - Every module that needs an Apollo credit number (preflight estimation,
 *     cost guardrails, usage logging, reconciliation) reads it from here.
 *   - No caller may hardcode `3`, `+ 1`, or `3 + 1`.
 *   - The breakdown always carries `pricingSource` and `pricingVersion` so a
 *     reconciliation can tell which pricing produced a reservation.
 *
 * Pure: no I/O and no process.env reads. Caps are injected by the caller so
 * this module is deterministic and testable offline.
 */
...
/** Canonical operation keys as written to provider_usage_logs.operation_key. */
...
/** Credits a single unit of the given operation costs. Used by usage logging. */
