...
/**
 * A1-APOLLO-BUDGET-RECONCILIATION-1: estos tests son sobre el cableado de la
 * captura de labels, pero el enrichment del que dependen es una llamada PAGADA,
 * y las llamadas pagadas ahora pasan por gates. Un sector sin mapping — incluido
 * "ningún sector" — ya no autoriza enrichment, así que las requests declaran uno
 * mapeado.
 *
 * Esto no debilita el aislamiento para el que se escribió el archivo: la captura
 * del lado Search se dispara con los campos de industria de la ORG, y
 * ORG_SEARCH_BARE sigue sin ninguno. Sólo cambió el sector declarado del request.
 */
...
describe('Q3F-5AU.12 — Apollo organization enrichment: raw industry label capture wiring', () => {
  withEnabledEnv(() => {
    ...
    it('T10b: missing countryCode/agentRunId fall back to null, same as Search-side wiring', async () => {
      ...
      // A1-APOLLO-BUDGET-RECONCILIATION-1: aquí ya no se omite la industria.
      // Omitirla ahora significa "no existe política de sector", lo que bloquea
      // correctamente el enrichment pagado — así que deja de servir para
      // ejercitar los fallbacks a null. countryCode y agentRunId sí siguen.
      await runApolloOrganizationsSearch(MAPPED_SECTOR_SEARCH_INPUT, 3, undefined, deps);
    ...
    it('T10c: una request SIN sector no ejecuta ningún enrichment pagado', async () => {
      // A1-APOLLO-BUDGET-RECONCILIATION-1: "no tenemos mapping de sector" nunca
      // debe autorizar gasto. Es el contrato del que T10b asumía lo contrario,
      // así que se afirma explícitamente en vez de quedar implícito.
      ...
      assert.equal(enrichCallCount, 0, 'sin sector ⇒ sin llamada pagada a Apollo');
      assert.equal(
