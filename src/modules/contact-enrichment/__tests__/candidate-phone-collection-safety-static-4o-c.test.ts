...
describe('4O-C-R1 — exactamente UNA migración nueva, y sin backfill', () => {
  ...
  it('la 110 sigue siendo la única migración de este hito, y el techo no se ha abierto', () => {
    ...
    assert.equal(
      ...
      // BR-SOURCE-FUNCTIONAL-CUT-A mueve el techo a la 125 originalmente: la identidad MENSUAL
      ...
      // dejar sitio a una migración 125 genérica y nueva (`125_reconcile_source_snapshot_record_identity.sql`):
      // reconcilia la unicidad genérica de `record_identity_key` sobre `source_company_snapshots`
      // para fuentes NO brasileñas. Tampoco es de teléfono, tampoco nombra ninguna tabla, columna
      // ni función de la cadena de teléfono, y la autoría de ambas se comprueba abajo archivo por
      // archivo. Ambas AUTORADAS y NO APLICADAS.
      '126_br_receita_monthly_snapshot_identity.sql',
      'el techo conocido es la 126 (identidad mensual del snapshot BR, renumerada por CUT A.1), que no edita la cadena de teléfono 109–117',
    );
    assert.equal(
      // La ventana sube con el techo DECLARADO arriba: la 125 (reconciliación genérica) y la 126
      // (BR, renumerada) están autorizadas y nombradas, así que lo prohibido pasa a ser la 127 y
      // superiores.
