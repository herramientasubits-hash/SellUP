...
| **Migo API** — API RUC Lookup | [api.migo.pe](https://api.migo.pe/) · [docs.migo.pe](https://docs.migo.pe/) | `validation_only` (NO CIIU, NO discovery) | Todos | RUC, razón social, estado del contribuyente, condición de domicilio, dirección, ubigeo. **⚠️ Spike real Perú.3N-R: NO devuelve CIIU ni actividad económica.** Verdict: `MIGO_NOT_USEFUL_FOR_CIIU`. Solo útil para validar RUC puntual bajo demanda. Puede ser redundante frente a SUNAT Padrón Reducido. | Alta — API REST con Bearer token | Demo gratis: 700q/7d. Básico: S/15/mes (40K). Empresa: S/25/mes (80K). Premium: S/25/mes (150K). | **P2** (opcional, solo validación RUC) |
