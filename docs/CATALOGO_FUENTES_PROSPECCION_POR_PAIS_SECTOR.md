...
El **Padrón RUC de SUNAT** es la mejor fuente pública de Perú: descarga ZIP diaria gratuita con todos los contribuyentes activos, condición "habido/no habido" como señal de calidad, ubigeo. **⚠️ Perú.3K:** El Padrón Reducido descargable tiene solo 15 columnas y NO incluye CIIU ni actividad económica — SUNAT tiene CIIU internamente pero no lo exporta en el archivo público. **⚠️ Perú.3L-2A:** PRODUCE MiPyme por Sector Productivo está **bloqueado por WAF** (CloudFront 403) — no hay URL estática machine-readable para datos 2022+. **⚠️ Perú.3M:** Migo API es la única fuente operable confirmada para CIIU masivo en Perú. Verdict: `SPIKE_WITH_TEST_KEY`. Estrategia MVP recomendada: SUNAT (base legal) + Migo API (CIIU bajo demanda, enrichment_provider). Limitación principal: ninguna fuente oficial gratuita incluye CIIU operable; CIIU masivo requiere API privada (Migo API, S/15–25/mes).

### Fuentes recomendadas — MVP activo

| Fuente | URL | Tipo | Sectores útiles | Uso recomendado | Automatización MVP | Riesgos / límites | Prioridad |
|---|---|---|---|---|---|---|---|
| **SUNAT — Padrón RUC** (descarga masiva) | [sunat.gob.pe — padrón](https://www.sunat.gob.pe/descargaPRR/mrc137_padron_reducido.html) · [datosabiertos.gob.pe](https://www.datosabiertos.gob.pe/dataset/padr%C3%B3n-ruc-superintendencia-nacional-de-aduanas-y-de-administraci%C3%B3n-tributaria-sunat) | Discovery, Validación tributaria, Duplicidad | Todos | ZIP periódico: RUC, razón social, estado, condición habido/no habido, ubigeo. ⚠️ **NO incluye CIIU** (Perú.3K: confirmado — 15 columnas sin actividad económica). Actualización diaria 23:00. **Base del MVP Perú para identidad + estado.** | Alta — ZIP gratuito; conector implementado en SellUp | Sin CIIU, sin contacto (teléfono/email). Padrón "reducido" no es versión recortada del completo — es la única versión pública disponible. | **P0** |
| **SUNAT — Consulta RUC individual** | [e-consultaruc.sunat.gob.pe](https://e-consultaruc.sunat.gob.pe/) | Validación tributaria | Todos | Validar RUC puntual, razón social, estado. Sí incluye CIIU pero requiere captcha manual | Manual — posible captcha | No automatizar scraping | **P0** |
| **PRODUCE — MiPyme por Sector Productivo** ⭐ | [datosabiertos.gob.pe — MiPyme](https://www.datosabiertos.gob.pe/dataset/directorio-de-empresas-mipyme-por-sector-productivo-ministerio-de-la-producci%C3%B3n-produce) | Enriquecimiento CIIU | Manufactura, Comercio, Servicios | **Fuente oficial RUC + CIIU para MiPymes formales.** Descarga directa CSV/Excel. Cruzable con snapshot SUNAT por RUC. ⭐ Mejor fuente oficial gratuita para CIIU masivo. | Alta — descarga CSV directa | Solo MiPymes (no grandes empresas). Datos anuales. Cobertura pendiente de verificar en spike Perú.3L. | **P0** (para CIIU) |
| **PRODUCE — Directorio Grandes Empresas Manufactura** | [datosabiertos.gob.pe — manufactura](https://www.datosabiertos.gob.pe/dataset/directorio-de-grandes-empresas-del-sector-manufactura) | Enriquecimiento | Manufactura | Grandes empresas manufactureras: RUC, ubigeo, CIIU. Complementa MiPyme para grandes empresas manufactura. | Alta — descarga CSV directa | Solo grandes empresas manufactura. Cobertura sectorial parcial. | **P1** (sector manufactura) |
...
### Fuentes post-MVP / investigación adicional

| Fuente | URL | Tipo | Sectores útiles | Uso recomendado | Automatización MVP | Riesgos / límites | Prioridad |
|---|---|---|---|---|---|---|---|
| **OpenRUC** — API RUC gratuita | [openruc.com](https://openruc.com/) | API privada RUC (gratis) | Todos | Fallback de validación RUC puntual cuando el bulk no tiene el RUC. API gratuita sin auth, datos oficiales SUNAT. Patrocinado por Latinfo. | Alta — API REST sin auth | Sin SLA formal. Datos públicos SUNAT. Open source (GitHub). | **P1** (post-MVP) |
| **Latinfo** — Plataforma multi-fuente | [latinfo.dev/pe](https://latinfo.dev/pe) | API privada enriquecimiento | Todos | RUC + sanciones (OSCE, OEFA) + deuda coactiva SUNAT + contratación SEACE + RNP + score KYB. Cruzas múltiples fuentes en una API. | Alta — API REST con key | 1,000 créditos/mes gratis. Cubre PE, CO, CL, BR, EC, AR. Proveedor privado. | **P1** (post-MVP) |
| **Migo API** — API RUC + CIIU ⭐ | [api.migo.pe](https://api.migo.pe/) · [docs.migo.pe](https://docs.migo.pe/) | `enrichment_provider` (NO discovery) | Todos | RUC + CIIU Rev3/Rev4 (principal + secundarias) + estado + condición + ubigeo + representantes legales. **Perú.3M verdict: `SPIKE_WITH_TEST_KEY`** — mejor proveedor privado confirmado para CIIU masivo Perú. Estrategia MVP: SUNAT (base legal) + Migo (CIIU bajo demanda). Worker/job, no Vercel. NO guardar representantes (datos personales). | Alta — API REST con Bearer token | Demo gratis: 700q/7d. Básico: S/15/mes (40K). Empresa: S/25/mes (80K). Premium: S/25/mes (150K). ToS no revisados para IA/agentes — revisar antes de producción. | **P0** (MVP CIIU, previo spike) |
| **Perú API** — API RUC/DNI | [peruapi.com](https://peruapi.com/) | API privada RUC | Todos | RUC + sucursales. API REST con API Key. Caché inteligente con Memcached. | Alta — API REST con API Key | Free: 100/día, 1000/mes. Basic desde pago. Documentación completa. | **P1** (post-MVP) |
| **ApiPeru.dev** — API RUC/DNI/CPE | [apiperu.dev](https://apiperu.dev/) · [docs.apiperu.dev](https://docs.apiperu.dev/) | API privada RUC | Todos | RUC + representantes legales + establecimientos anexos + CPE + comisiones AFP. | Alta — API REST con Bearer token | Free: 100/mes. Micro S/5/mes (2,500 consultas). Planes desde S/15/mes. | **P1** (post-MVP) |
| **RNP** — Registro Nacional de Proveedores | [osce.gob.pe — RNP](https://www.osce.gob.pe/) | Registro oficial | Todos B2G | Proveedores habilitados para contratar con el Estado: RUC, especialidad, estado. | Baja — consulta web individual | Sin API pública documentada. Complementa SEACE. | **P1** (post-MVP) |

### Fuentes manuales / sectoriales

| Fuente | URL | Tipo | Sectores útiles | Uso recomendado | Automatización MVP | Riesgos / límites | Prioridad |
|---|---|---|---|---|---|---|---|
| **SUNARP** — Personas Jurídicas | [sunarp.gob.pe — búsqueda](https://www.sunarp.gob.pe/bus-personas-juridicas.asp) | Validación legal | Todos | Sociedades inscritas: nombre, RUC, representante legal, vigencia | Manual — consulta individual | Sin descarga masiva documentada | **P1** |
...
| **Perú** | SUNAT Padrón RUC (descarga ZIP) | ZIP diario gratuito. RUC + estado habido/no habido. ⚠️ **NO incluye CIIU** (Perú.3K). Base de identidad + estado. | Alta |
| **Perú** | PRODUCE MiPyme por Sector Productivo ⭐ | Descarga CSV gratuita. **RUC + CIIU** para MiPymes en Manufactura, Comercio, Servicios. Fuente oficial para CIIU masivo. | Alta |
| **Perú** | PRODUCE Grandes Empresas Manufactura | Open data segmentado por manufactura. RUC + CIIU grandes empresas. | Alta |
