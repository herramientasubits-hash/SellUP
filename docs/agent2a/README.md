...
> **Estado:** funcionalmente terminado y operativo en Producción.
...
## 4. Snapshot de Producción (READ-ONLY, 2026-08-19)
...
| Regla de crédito Apollo | presente (4 reglas) — **ninguna activa** |
| Regla de crédito Lusha | presente y **activa** (scope `user`, mensual, en créditos, `on_exceed=block`) |

> La consecuencia operativa de «ninguna regla Apollo activa» está documentada en
> [OPERATIONS_RUNBOOK.md](OPERATIONS_RUNBOOK.md) § E. No es un defecto del código: es el
> comportamiento *fail-closed* previsto por `budget_not_configured`.
...
| El cuerpo del PR #309 afirma «En Producción no hay ninguna regla de crédito activa para Lusha». Hoy **sí la hay** (fue configurada después, y es lo que permitió la QA de aceptación). | `budget_rules` leído en vivo |
