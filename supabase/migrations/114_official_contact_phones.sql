...
-- is today. The legacy mobile scalar on `contacts` is likewise untouched and stays
-- transitional (4O-H0 decision; convergence belongs to H5). There is deliberately NO trigger syncing the
...
-- to make provenance demonstrable. The legacy mobile scalar is worse still: it has no
