-- Migration 100: Spend-correlation columns on provider_usage_logs
...
--
-- IMPORTANT — this migration is NOT required for the feature to work.
-- The application writes the same values into
-- `provider_usage_logs.metadata.run_correlation`, and the reconciliation reads
-- columns first and metadata second. These columns exist only to make the
-- queries indexable. Writing them is gated behind
-- ENABLE_PROVIDER_USAGE_CORRELATION_COLUMNS, which stays OFF until this
-- migration has been applied. Flipping the flag needs no backfill: historic
-- rows keep answering from metadata.
--
...
--   * no FK on reservation_id: provider_usage_logs is an append-only ledger, and
--     a released reservation must never block or delete spend history
--
...
-- an enum type, so adding a state later is another additive migration instead
-- of an ALTER TYPE on a live enum.
ALTER TABLE public.provider_usage_logs
